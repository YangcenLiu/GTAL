...
class Inverse_BWA_fusion_dropout_feat_v2(nn.Module):
    def __init__(self, n_feature, n_class, **args):
        super().__init__()
        embed_dim = 1024
        self.channel_conv = nn.Sequential(
        ...
        self.decoder = nn.Sequential(
            nn.Conv1d(embed_dim, n_feature, (3,), padding=1), 
            nn.LeakyReLU(0.2), 
            nn.Dropout(0.5)
        )

    def forward(self, filter_feat, vfeat):
        channelfeat = self.channel_avg(vfeat)
        channel_attn = self.channel_conv(channelfeat)
        
        bit_wise_attn = self.bit_wise_attn(filter_feat)

        fused_feat = torch.sigmoid(bit_wise_attn * channel_attn)
        ffeat = self.decoder(fused_feat)
...
class DELU_CROSS(torch.nn.Module):
    def __init__(self, n_feature, n_class, device="cuda:0", **args):
        super().__init__()
        embed_dim = 2048
        mid_dim = 1024
        ...
        self.Inverse_vAttn = Inverse_BWA_fusion_dropout_feat_v2(1024, args)
        self.Inverse_fAttn = Inverse_BWA_fusion_dropout_feat_v2(1024, args)
    ...
    def forward(self, inputs, is_training=True, **args):
        ...
        pred_vfeat = self.Inverse_vAttn(vfeat, feat[:, :1024, :])
        pred_vfeat = self.Inverse_vAttn(ffeat, feat[:, 1024, :])
