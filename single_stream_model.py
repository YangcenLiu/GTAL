...
class DELU(torch.nn.Module):
    def __init__(self, n_feature, n_class, **args):
        ...
        self.vAttn = getattr(single_stream_model, args["opt"].AWM)(512, args)
        self.fAttn = getattr(single_stream_model, args["opt"].AWM)(512, args)
    ...
    def forward(self, inputs, is_training=True, **args):
        ...
        v_atn, vfeat = self.vAttn(feat[:, :512, :], feat[:, 512:, :])
        f_atn, ffeat = self.fAttn(feat[:, 512:, :], feat[:, :512, :])
        x_atn = (f_atn + v_atn) / 2
    ...
    def criterion(self, outputs, labels, **args):
        feat, element_logits, element_atn = (
            outputs["feat"],
            outputs["cas"],
            outputs["attn"],
        )
        v_atn = outputs["v_atn"]
        f_atn = outputs["f_atn"]
        mutual_loss = 0.5 * F.mse_loss(v_atn, f_atn.detach()) + 0.5 * F.mse_loss(
            f_atn, v_atn.detach()
        )
        ...
        loss_mil_orig, _ = self.topkloss(
            element_logits, labels, is_back=True, rat=args["opt"].k
        )

        # SAL
        loss_mil_supp, _ = self.topkloss(
            element_logits_supp, labels, is_back=False, rat=args["opt"].k
        )
        ...
        # guide loss
        loss_guide = (
            (1 - element_atn - element_logits.softmax(-1)[..., [-1]]).abs().mean()
        )
        ...
        loss_dict = {
            ...
            "norm_loss": args["opt"].alpha1
            * (loss_norm + v_loss_norm + f_loss_norm)
            / 3,
            "guide_loss": args["opt"].alpha2
            * (loss_guide + v_loss_guide + f_loss_guide)
            / 3,
    ...
    def uct_guide_loss(
        ...
        loss_guide = (
            (1 - element_atn - element_logits.softmax(-1)[..., [-1]]).abs().squeeze()
        )

        v_loss_guide = (
            (1 - v_atn - element_logits.softmax(-1)[..., [-1]]).abs().squeeze()
        )

        f_loss_guide = (
            (1 - f_atn - element_logits.softmax(-1)[..., [-1]]).abs().squeeze()
        )
    ...
    def edl_loss(
        ...
        topk_element_logits = torch.gather(element_logits_supp, 1, atn_idx_expand)[
            :, :, :-1
        ]
        ...
        edl_results = edl_loss(
            output=video_logits, target=labels, epoch=epoch, total_epoch=total_epoch
        )
    ...
    def topkloss(self, element_logits, labels, is_back=True, rat=8):
        ...
        topk_val, topk_ind = torch.topk(
            element_logits, k=max(1, int(element_logits.shape[-2] // rat)), dim=-2
        )
        ...
        milloss = -(labels_with_back * F.log_softmax(instance_logits, dim=-1)).sum(
            dim=-1
        )
    ...
    def Contrastive(self, x, element_logits, labels, is_back=False):
        ...
        for i in range(0, 3 * 2, 2):
            ...
            sim_loss = sim_loss + 0.5 * torch.sum(
                torch.max(d1 - d2 + 0.5, torch.FloatTensor([0.0]).cuda())
                * labels[i, :]
                * labels[i + 1, :]
            )
            sim_loss = sim_loss + 0.5 * torch.sum(
                torch.max(d1 - d3 + 0.5, torch.FloatTensor([0.0]).cuda())
                * labels[i, :]
                * labels[i + 1, :]
            )
...
class DELU_ACT(torch.nn.Module):
    def __init__(self, n_feature, n_class, **args):
        ...
        self.pool = (
            nn.AvgPool1d(_kernel, 1, padding=_kernel // 2, count_include_pad=True)
    ...
    def criterion(self, outputs, labels, **args):
        ...
        loss_mil_orig, _ = self.topkloss(
            element_logits, labels, is_back=True, rat=args["opt"].k, reduce=None
        )
        # SAL
        loss_mil_supp, _ = self.topkloss(
            element_logits_supp, labels, is_back=False, rat=args["opt"].k, reduce=None
        )
    ...
    def topkloss(
        self, element_logits, labels, is_back=True, lab_rand=None, rat=8, reduce=None
    ):
